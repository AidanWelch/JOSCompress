...
class Schema {
     ...
     */
    constructor(schema){
        if(Array.isArray(schema) || (typeof message === 'object' && message !== null)){
            this.schema = schema;
        } else {
    ...
    decode(message, permissive = false){
                ...
                switch (string[i]) {
                    case ',':
                        if(!in_string && !array_nesting && !object_nesting){
                            result = string.slice(i), string.slice(0, i);
                        }
                        escaped = false;
                        break;
        ...
        function decodeObject(object, schm){
            ...
            while(object.length != 0){
                let property;
                object, property = extractProperty(object, schm);
        ...
        function decodeArray(array, schm){
            ...
            while(array.length != 0){
                let property;
                array, property = extractProperty(array, schm[0]);
                result.push(property);
            }
            return result;
        }

        function schemaIdentify(schm, string){
            if(Array.isArray(schm)){
                return decodeArray(string, schm);
            } else if (typeof schm === 'object' && schm !== null) {
                return decodeObject(string, schm);
            } else {
                return string;
            }
        }

        return schemaIdentify(this.schema, message);
    }
}
